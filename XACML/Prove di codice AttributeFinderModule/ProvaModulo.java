  ...
  */
  ...
  public EvaluationResult findAttribute(URI attributeType, URI attributeId, URI issuer, URI subjectCategory, EvaluationCtx context, int designatorType) {
    ...
    if (designatorType != AttributeDesignator.RESOURCE_TARGET) {
    			// assicuriamoci che l'attributo sia del tipo giusto (in questo esempio è di
    			// tipo resource)
    			...
    			// qui estraiamo le informazioni necessarie dai valori passati alla funzione
    			// findAttribute
    			 ...
    			 *
    			 * // -ricavo le informazioni nella richiesta utilizzando il context
    			 * (principalmente info dalla richiesta) // -in pratica viene chiamato
    			 * l'AttributeFinderModule di base per ricavare gli AttributeValue dalla
    			 * richiesta // EvaluationResult getResourceAttribute(URI type, URI id, URI
    			 * issuer) EvaluationResult attr_1_res = context.getResourceAttribute(type, id,
    			 * issuer); EvaluationResult attr_2_res = context.getResourceAttribute(type, id,
    			 * issuer);
    			 *
    			 * // ricavo gli AttributeValue (StringAttribute o IntegerAttribute) dagli
    			 * oggetti EvaluationResult StringAttribute attr_1_at = (StringAttribute)
    			 * attr_1_res.getAttributeValue(); IntegerAttribute attr_2_at =
    			 * (IntegerAttribute) attr_2_res.getAttributeValue();
    			 *
    			 * // estraggo stringhe e interi dagli AttributeValue String attr_1_st =
    			 * attr_1_at.getValue(); int attr_2_int = (int) attr_2_at.getValue();
    			 */
    			...
    			// -sulla base delle informazioni ricavate, di seguito vanno utilizzate le
    			// funzioni di AceQL
    			...
    			// stub (la collection verrà costruita utilizzando le funzioni di query dal DB
    			// di AceQL)
